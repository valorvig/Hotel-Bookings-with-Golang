...
// GetRestrictionsForRoomByDatereturns restricitons for a room by date range
func (m *postgresDBRepo) GetRestrictionsForRoomByDate(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&r.ID,
			&r.ReservationID, // we might not always have the reservation - null; Go will not allow that because it's not an int
			&r.RestrictionID,
			&r.RoomID,
			&r.StartDate,
			&r.EndDate,
...
func (m *postgresDBRepo) DeleteBlockByID(id int) error {
	...
	query := `delete from room_restricitons where id = $1`
